...
namespace Gifer {
    public partial class MainForm : Form {
        ...
        async private void CutGif(int from, int to, String filePath, String[] additionalFilter = null, bool subtitles = false, bool fullResolution = false, bool keepAudio = false) {
            ...
            if (subtitles) {
                if (isDVDsubs) {
                    var dvdSubsVf = FormattableString.Invariant($"scale[vidi];[0:{selectedSubtitle.Index}]fps=fps={videoStream.Framerate}[sub];[sub][vidi]scale2ref[subrs][vidio];[vidio][subrs]overlay");
                    vfs.Add(dvdSubsVf);
                } else {
                    vfs.Add(subtitlesVf);
                }
